...
module.exports = {
  ...
  getGraphQLSchema: function (params) {
    ...
    const Mutation = this.getMutations();
          ...
          if (rules.hasOwnProperty('model')) {
            ...
            fields[key] = {
              ...
              resolve: function (object) {
                const criteria = {};
                criteria[rules.via.toLowerCase()] = object.id.toString();
  ...
  buildQueryFields: function (collection) {
    ...
    fields[collectionIdentity.toLowerCase() + 's'] = {
      ...
      resolve: function (rootValue, criteria, parent) {
  ...
  buildMutation: function (collection) {
    ...
    _.forEach(collectionAttributes, function (rules, key) {
      // Exclude relations
      if (!rules.hasOwnProperty('model') && !rules.hasOwnProperty('collection')) {
        args.required[key] = {
          type: rules.required ? new GraphQL.GraphQLNonNull(convertToGraphQLQueryType(rules, collection.primaryKey)) : convertToGraphQLQueryType(rules, collection.primaryKey)
        };
        ...
        args.required[key] = {
          type: rules.required ? new GraphQL.GraphQLNonNull(convertToGraphQLRelationType(rules, collectionAttributes[collection.primaryKey])) : convertToGraphQLRelationType(rules, collectionAttributes[collection.primaryKey])
        };
      }
    });
    ...
    // Set primary key as required for update mutation
    argPK[collection.primaryKey] = {
      type: new GraphQL.GraphQLNonNull(convertToGraphQLQueryType(collectionAttributes[collection.primaryKey]))
    };
    ...
    fields['update' + collectionIdentity] = {
      ...
      resolve: function (rootValue, args) {
        return collection.update(args[collection.primaryKey], args);
      }
    };
    ...
    fields['delete' + collectionIdentity] = {
      type: new GraphQL.GraphQLList(this.defaults.types[collectionIdentity]),
      args: args.notRequired,
      resolve: function (rootValue, args) {
        return collection.destroy(args);
      }
    };
  ...
  buildNodeInterface: function () {
    ...
    this.defaults.nodeFields = {
      ...
      resolve: function resolve(rootValue, criteria) {
...
/*
 * Convert Waterline type to GraphQL type system for input fields
 *
 * @return {Object}
 */
